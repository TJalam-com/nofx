...
// PositionSnapshot represents core data of current positions, used for backtest state and persistence.
...
// BacktestState represents real-time state during execution (in-memory state).
...
// TradeEvent records a trade execution result or special event (e.g., liquidation).
...
// Metrics aggregates backtest performance metrics.
type Metrics struct {
...
// Checkpoint represents checkpoint information saved to disk, used for pause, resume, and crash recovery.
...
// RunMetadata records summary required for run.json.
...
// RunSummary is the summary field in run.json.
...
// StatusPayload used for /status API response.
