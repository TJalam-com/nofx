...
// NewRunner builds a backtest runner.
...
// Start starts the backtest loop.
...
// PersistMetadata writes current snapshot to run.json.
...
// CurrentMetadata returns metadata corresponding to current in-memory state.
		...
		if r.aiCache != nil {
			if key, err := computeCacheKey(ctx, r.cfg.PromptVariant, ts); err == nil {
				...
				if cached, ok := r.aiCache.Get(cacheKey); ok {
					...
					record.ErrorMessage = fmt.Sprintf("No cached decision found for ts=%d", ts)
...
func (r *Runner) fillDecisionRecord(record *logger.DecisionRecord, full *decision.FullDecision) {
	...
	record.RawResponse = full.RawResponse // Save raw AI response for debugging parse failures
...
func (r *Runner) executeDecision(dec decision.Decision, priceMap map[string]float64, ts int64, cycle int) (logger.DecisionAction, []TradeEvent, string, error) {
	...
	case "hold", "wait":
		return actionRecord, nil, fmt.Sprintf("Maintain position: %s", dec.Action), nil
	default:
...
// StatusPayload builds status response for API.
