...
interface ArticleFormProps {
  ...
  onCancel: () => void
  isLoading?: boolean
  onFormChange?: (formData: { title: string; content: string; excerpt: string; slug: string }) => void
}
...
export function ArticleForm({ article, onSubmit, onCancel, isLoading = false, onFormChange }: ArticleFormProps) {
  ...
  }, [title, article]) // Note: intentionally not including slug and isSlugManuallyEdited to avoid loops
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (status === 'published') {
      ...
      if (missingFields.length > 0) {
        toast.error(`Cannot publish: Missing required fields: ${missingFields.join(', ')}. Please fill in all required fields before publishing.`)
        return
      }
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label htmlFor="slug" className="block text-sm font-medium mb-2" style={{ color: '#000000' }}>
          ...
          {article && <span className="ml-2 text-xs text-gray-500">(Cannot be changed after creation)</span>}
        </label>
        <input
          type="text"
          id="slug"
          ...
          style={{ 
            borderColor: 'var(--border-color, #d1d5db)', 
            color: '#000000', 
            backgroundColor: article ? 'var(--bg-secondary, #f3f4f6)' : 'var(--bg-primary, #ffffff)' 
          }}
        />
      </div>
      ...
      <div>
        ...
        <div className="border rounded-md" style={{ borderColor: 'var(--border-color, #d1d5db)' }}>
