...
// GetCoinPool gets coin pool list (with retry and cache mechanism)
func GetCoinPool() ([]CoinInfo, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		if attempt > 1 {
			log.Printf("⚠️  Retrying coin pool fetch (attempt %d/%d)...", attempt, maxRetries)
	...
	// API fetch failed, try using cache
...
// GetAvailableCoins gets available coin list (filter out unavailable ones)
func GetAvailableCoins() ([]string, error) {
	...
	for _, coin := range coins {
		if coin.IsAvailable {
			// Ensure symbol format is correct (convert to uppercase USDT trading pair)
...
// GetTopRatedCoins gets top N rated coins (sorted by score from high to low)
func GetTopRatedCoins(limit int) ([]string, error) {
	...
	// Sort by Score in descending order (bubble sort)
...
// convertSymbolsToCoins converts coin symbol list to CoinInfo list
...
// ========== OI Top (Top 20 Open Interest Growth) Data ==========
...
// GetOITopPositions gets Top 20 open interest growth data (with retry and cache)
func GetOITopPositions() ([]OIPosition, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		if attempt > 1 {
			log.Printf("⚠️  Retrying OI Top data fetch (attempt %d/%d)...", attempt, maxRetries)
...
// GetOITopSymbols gets OI Top coin symbol list
...
type MergedCoinPool struct {
	AI500Coins    []CoinInfo          // AI500 rated coins
	OITopCoins    []OIPosition        // Top 20 open interest growth
	AllSymbols    []string            // All unique coin symbols
	SymbolSources map[string][]string // Source for each coin ("ai500"/"oi_top")
}

// GetMergedCoinPool gets merged coin pool (AI500 + OI Top, deduplicated)
func GetMergedCoinPool(ai500Limit int) (*MergedCoinPool, error) {
	...
	log.Printf("📊 Coin pool merge completed: AI500=%d, OI_Top=%d, Total(dedup)=%d",
