...
// Trusted domains that are explicitly allowed (e.g., own deployment domain)
...
func ValidateURL(rawURL string) error {
	...
	// Check if it's a trusted domain (explicitly allow own deployment domain)
	for _, trusted := range trustedDomains {
		if lowerHost == trusted || strings.HasSuffix(lowerHost, "."+trusted) {
			return nil // Trusted domain, allow it
		}
	}
