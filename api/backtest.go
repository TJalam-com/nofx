...
func (s *Server) hydrateBacktestAIConfig(cfg *backtest.BacktestConfig) error {
	...
	if modelID != "" {
		...
		if err != nil {
			return fmt.Errorf("get default ai model: %w", err)
		}
	...
	if cfg.AICfg.Provider == "custom" {
		if cfg.AICfg.BaseURL == "" {
			return fmt.Errorf("custom ai model api url not configured")
		}
		if cfg.AICfg.Model == "" {
			return fmt.Errorf("custom ai model name not configured")
		}
	}
