...
// Verify that unique traderIDs can be generated even under the same exchange and AI model
func TestTraderIDUniqueness(t *testing.T) {
	exchangeID := "binance"
	aiModelID := "gpt-4"

	// Simulate creating 100 traders simultaneously (same parameters)
...
func isValidTraderIDFormat(traderID, expectedExchange, expectedModel string) bool {
	...
	// AI model may contain hyphens (e.g., gpt-4), so need to reassemble
	// Extract model part (parts[1] to parts[len(parts)-2])
...
func TestTraderIDFormat(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify contains correct prefix
			...
			// Verify length is reasonable (should have at least exchange + model + "_" + UUID(36) length)
			minLength := len(tt.exchangeID) + len(tt.aiModelID) + 2 + 36 // 2 underscores + 36 char UUID
...
// TestTraderIDNoCollision Test no collision under high concurrency scenarios
