...
func GetTradingViewJSONFormat() string {
	...
	sb.WriteString("  \"confidence\": 78,\n")
	sb.WriteString("  \"risk_usd\": 150,\n")
	sb.WriteString("  \"reasoning\": \"Signal aligns with bearish divergence on 4H; risk-reward 2.5:1 acceptable\"\n")
	sb.WriteString("}\n")
	sb.WriteString("```\n")
	sb.WriteString("</decision>\n\n")
	...
	sb.WriteString("- `confidence`: **REQUIRED** (0-100). Your independent assessment of trade quality based on your own analysis of market data, indicators, and the signal parameters. Do NOT blindly accept or copy any external confidence — calculate this yourself by evaluating signal alignment with current market conditions, risk-reward, trend strength, and indicator confluence.\n")
	sb.WriteString("- `risk_usd`: Maximum USD risk for this trade\n")
	sb.WriteString("- If `signal_decision` is \"reject\", you do not need to provide other trading parameters\n")
	sb.WriteString("- If `signal_decision` is \"accept\", you must provide complete trading parameters (symbol, action, leverage, position_size_usd, stop_loss, take_profit, confidence, risk_usd, etc.) and `stop_loss`/`take_profit` MUST be exactly equal to the SL/TP values from the TradingView signal\n")
	sb.WriteString("- If `signal_decision` is \"modify\", you may ONLY adjust `leverage` and `position_size_usd` based on risk management. `stop_loss` and `take_profit` MUST remain exactly equal to the SL/TP values from the TradingView signal and cannot be widened, tightened, or moved.\n\n")
	sb.WriteString("**IMPORTANT: All numeric values in your JSON decision output must be calculated numbers, not formulas or expressions.**\n")
	sb.WriteString("- ✅ Correct: `\"position_size_usd\": 1500`, `\"stop_loss\": 130.00`, `\"leverage\": 3`, `\"confidence\": 78`\n")
	sb.WriteString("- ❌ Wrong: `\"position_size_usd\": \"available_margin * 3\"`, `\"stop_loss\": \"entry * 1.05\"`, `\"confidence\": 0`\n\n")
...
func GetParentSignalJSONFormat() string {
	...
	sb.WriteString("  \"confidence\": 80,\n")
	sb.WriteString("  \"risk_usd\": 200,\n")
	sb.WriteString("  \"reasoning\": \"Scaling down position size for my account, signal direction confirmed by 4H trend\"\n")
	sb.WriteString("}\n")
	sb.WriteString("```\n")
	sb.WriteString("</decision>\n\n")
	sb.WriteString("## Field Requirements\n\n")
	...
	sb.WriteString("- `confidence`: **REQUIRED** (0-100). Your independent assessment of trade quality based on your own analysis of market data, indicators, and the signal parameters. Do NOT copy the parent's confidence — calculate this yourself.\n")
	sb.WriteString("- `risk_usd`: Maximum USD risk for this trade\n")
	sb.WriteString("- If `signal_decision` is \"reject\", you do not need to provide other trading parameters\n")
	sb.WriteString("- If `signal_decision` is \"accept\" or \"modify\", you must provide complete trading parameters (symbol, action, leverage, position_size_usd, stop_loss, take_profit, confidence, risk_usd, etc.)\n\n")
	sb.WriteString("**IMPORTANT: All numeric values in your JSON decision output must be calculated numbers, not formulas or expressions.**\n")
	sb.WriteString("- ✅ Correct: `\"position_size_usd\": 2200`, `\"stop_loss\": 95000`, `\"leverage\": 5`, `\"confidence\": 80`\n")
	sb.WriteString("- ❌ Wrong: `\"position_size_usd\": \"parent_size * 0.5\"`, `\"stop_loss\": \"parent_sl\"`, `\"confidence\": 0`\n\n")
