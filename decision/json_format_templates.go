...
func GetTradingViewJSONFormat() string {
	...
	sb.WriteString("- If `signal_decision` is \"accept`\", you must provide complete trading parameters (symbol, action, leverage, position_size_usd, stop_loss, take_profit, etc.) and `stop_loss`/`take_profit` MUST be exactly equal to the SL/TP values from the TradingView signal\n")
