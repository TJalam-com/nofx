...
// Config log configuration (simplified)
type Config struct {
...
// TelegramConfig Telegram push configuration (simplified, advanced parameters use default values)
type TelegramConfig struct {
	Enabled  bool   `json:"enabled"`   // Enabled (default: false)
	BotToken string `json:"bot_token"` // Bot Token
	ChatID   int64  `json:"chat_id"`   // Chat ID
	MinLevel string `json:"min_level"` // Minimum log level, logs at or above this level will be pushed to Telegram (optional, default: error)
}
...
// GetLogrusLevels returns log levels to push to Telegram
// Returns all log levels at or above the configured MinLevel
