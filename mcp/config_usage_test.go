...
func TestConfig_RetryWaitBase_IsUsed(t *testing.T) {
	...
	// 3rd attempt succeeds, but first 2 failed
	if err != nil {
	...
	// Total wait time should be approximately 3s (allow some tolerance)
...
func TestConfig_RetryableErrors_IsUsed(t *testing.T) {
	...
	// Modify config's RetryableErrors (no WithRetryableErrors option for now)
