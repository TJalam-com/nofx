...
func (client *Client) buildRequest(url string, jsonData []byte) (*http.Request, error) {
	...
	// Set auth header via hooks (supports subclass override)
	client.hooks.setAuthHeader(req.Header)

	return req, nil
}

// call single call to AI API (fixed flow, cannot be overridden)
func (client *Client) call(systemPrompt, userPrompt string) (string, error) {
	...
	// Step 1: Build request body (dynamic dispatch via hooks)
	requestBody := client.hooks.buildMCPRequestBody(systemPrompt, userPrompt)

	// Step 2: Serialize request body (dynamic dispatch via hooks)
	jsonData, err := client.hooks.marshalRequestBody(requestBody)
	if err != nil {
		return "", err
	}

	// Step 3: Build URL (dynamic dispatch via hooks)
	...
	// Step 8: Parse response (dynamic dispatch via hooks)
...
// callWithRequest single call to AI API (using Request object)
func (client *Client) callWithRequest(req *Request) (string, error) {
	// Print current AI configuration
